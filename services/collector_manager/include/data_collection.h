...
private:
    ...
    // 以下辅助方法供启停操作内部复用，均不持有 opMutex_（调用方负责持有）
    ...
    // mutex_ 仅保护簿记容器（eventIdToLoaderMap_/eventIdToSubscribeCount_）；dlopen、collector->Start/Stop
    // /Subscribe/Unsubscribe 等外部代码一律在 opMutex_ 下、mutex_ 之外执行。
    // 回调路径（采集器线程 -> ICollectorFwk::OnNotify）不取本锁，且本锁永不在持 mutex_ 时获取，故无锁环。
    ffrt::mutex opMutex_{};
