...
void Statement::Bind(const int32_t index, int32_t value)
{
    ...
    if (sqlite3_bind_int(statement_, index, value) != SQLITE_OK) {
        SGLOGE("Cannot bind int32_t, errorMsg: %{public}s", sqlite3_errmsg(db_));
    }
}
...
void Statement::Bind(const int32_t index, int64_t value)
{
    ...
    if (sqlite3_bind_int64(statement_, index, value) != SQLITE_OK) {
        SGLOGE("Cannot bind int64_t, errorMsg: %{public}s", sqlite3_errmsg(db_));
    }
}
...
{
    const char* textPtr = reinterpret_cast<const char*>(sqlite3_column_text(statement_, column));
    return textPtr ? std::string(textPtr) : std::string();
}
