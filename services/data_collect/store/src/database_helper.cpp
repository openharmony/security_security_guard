...
int DatabaseHelper::InsertEvent(const SecEvent& event)
{
    ...
    if (ret != NativeRdb::E_OK) {
        SGLOGI("failed to add event, eventId=%{public}lld, ret=%{public}d", static_cast<long long>(event.eventId), ret);
        return DB_OPT_ERR;
    }
    return SUCCESS;
}
...
int DatabaseHelper::QueryRecentEventByEventId(const std::vector<int64_t> &eventIds, std::vector<SecEvent> &events)
{
    ...
    for (int i = 0; i < size; i++) {
        SGLOGI("eventId=%{public}lld", static_cast<long long>(eventIds[i]));
        NativeRdb::RdbPredicates predicates(dbTable_);
...
int64_t DatabaseHelper::CountEventByEventId(int64_t eventId)
{
    ...
    int ret = Count(count, predicates);
    if (ret != NativeRdb::E_OK) {
        SGLOGE("failed to count event, eventId=%{public}lld, ret=%{public}d", static_cast<long long>(eventId), ret);
    }
    return count;
}
...
int DatabaseHelper::DeleteOldEventByEventId(int64_t eventId, int64_t count)
{
    ...
    if (resultSet == nullptr) {
        SGLOGI("failed to get event, eventId=%{public}lld", static_cast<long long>(eventId));
        return DB_OPT_ERR;
    }
    ...
    if (ret != NativeRdb::E_OK) {
        SGLOGE("failed to delete event, eventId=%{public}lld, ret=%{public}d", static_cast<long long>(eventId), ret);
        return DB_OPT_ERR;
    }
    return SUCCESS;
}
...
int DatabaseHelper::DeleteAllEventByEventId(int64_t eventId)
{
    ...
    if (ret != NativeRdb::E_OK) {
        SGLOGI("failed to delete event, eventId=%{public}lld, ret=%{public}d", static_cast<long long>(eventId), ret);
        return DB_OPT_ERR;
    }
    return SUCCESS;
}
