    ...
    SecurityModelCallBack &callback)
{
    ...
    TaskCallback func = [callback] (const std::shared_ptr<BaseTask>& baseTask) {
        ...
        std::unique_ptr<SecurityModelResult> result = std::make_unique<SecurityModelResult>(devId, modelId, status);
        if (callback != nullptr) {
            callback(result);
        }
    };
    std::shared_ptr<BaseTask> task = std::make_shared<RequestSecurityModelResultTask>(devId, modelId, func);
