...
#include "physical_machine_detection_model.h"
...
namespace {
    constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, 0xD002F10, "SG_PHY_MACH_DET" };
    const std::vector<int64_t> EVENTIDS = { 1011009005 };
...
PhysicalMachineDetectionModel::~PhysicalMachineDetectionModel()
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
}

int32_t PhysicalMachineDetectionModel::Init(std::shared_ptr<IModelManager> api)
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
    if (api == nullptr) {
...
std::string PhysicalMachineDetectionModel::GetResult()
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
...
int32_t PhysicalMachineDetectionModel::SubscribeResult(std::shared_ptr<IModelResultListener> listener)
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
    return SUCCESS;
}

void PhysicalMachineDetectionModel::Release()
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
}
...
extern "C" OHOS::Security::SecurityGuard::IModel *GetModelApi()
{
    OHOS::Security::SecurityGuard::IModel *api =
        new (std::nothrow) OHOS::Security::SecurityGuard::PhysicalMachineDetectionModel();
    return api;
}