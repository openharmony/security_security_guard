...
#include "device_completeness_model.h"
...
namespace {
    constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, 0xD002F10, "SG_DEVICE_COMP" };
    const std::vector<int64_t> EVENTIDS = { 1011009002, 1011009003, 1011009004 };
...
DeviceCompletenessModel::~DeviceCompletenessModel()
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
}

int32_t DeviceCompletenessModel::Init(std::shared_ptr<IModelManager> api)
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
    if (api == nullptr) {
...
std::string DeviceCompletenessModel::GetResult()
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
...
int32_t DeviceCompletenessModel::SubscribeResult(std::shared_ptr<IModelResultListener> listener)
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
    return SUCCESS;
}

void DeviceCompletenessModel::Release()
{
    HiLog::Info(LABEL, "func=%{public}s", __func__);
}
...
extern "C" OHOS::Security::SecurityGuard::IModel *GetModelApi()
{
    OHOS::Security::SecurityGuard::IModel *api =
        new (std::nothrow) OHOS::Security::SecurityGuard::DeviceCompletenessModel();
    return api;
}
